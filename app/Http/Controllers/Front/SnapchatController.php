...
use App\Models\User;
use App\Models\SnapchatCampaignModel;
use App\Models\WalletMasterModel;
use App\Models\SnapchatTransactionModel;
use App\Models\Channel;
use App\Models\SnapchatCampaignMediaModel;
...
/***********************************************************************************************/
// snapchat Campaign Functionality - By Yogesh Kumawat - Dated 31 May 2021 
/***********************************************************************************************/

    public function snapchat_create(){
    ...
    function store_snapchat_data(Request $request){    
        $campaign_id =0;
        ...
        if($request->hasFile('campaign_media')){
            foreach($request->file('campaign_media') as $file){
                $name = "snapchat-".time().rand(1,100).'.'.$file->extension();
    ...
    function load_snapchat_data($enc_id){
        ...
        if(!session()->has('LoggedUser')){
            ...
            $this->arr_view_data['user'] = $userID;
             $obj_user  = User::where('id',$userID)
                                   ->first();
            $this->arr_view_data['userData']  = $obj_user;              
            return view($this->module_view_folder.'.snapchat.campaign-list',$this->arr_view_data);
        }
    }
        ...
        if(!session()->has('LoggedUser')){
            ...
            $this->arr_view_data['user'] = $userID;
             $obj_user  = User::where('id',$userID)
                                   ->first();
            $this->arr_view_data['userData']  = $obj_user;     
            return view($this->module_view_folder.'.snapchat.transactions',$this->arr_view_data);
        }
    }
        ...
        if(!session()->has('LoggedUser')){
            ...
            $this->arr_view_data['user'] = $userID;
             $obj_user  = User::where('id',$userID)
                                   ->first();
            $this->arr_view_data['userData']  = $obj_user;              
            return view($this->module_view_folder.'.snapchat.report',$this->arr_view_data);
        }
    }
}